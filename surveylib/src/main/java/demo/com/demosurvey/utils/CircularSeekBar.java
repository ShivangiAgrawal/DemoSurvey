...
//        Log.e("CircularSeekBar", "onDraw: " + mProgressDegrees);
        ...
        canvas.drawPath(mCircleArrowPath, paint);
        canvas.drawPath(mCircleArrowSolidPath, paintSolid);
...
//((textPaint.descent() + textPaint.ascent()) / 2) is the distance from the baseline to the center.
        canvas.drawText(getProgress() + "", xPos, yPos, textProgress);

        Rect bounds = canvas.getClipBounds();
        StaticLayout sl = new StaticLayout(textCenter[getProgress()], textPaint, bounds.width(), Layout.Alignment.ALIGN_CENTER,
                1, 1, true);
        canvas.save();
        //text height and number of lines to move Y coordinate to center.
        float textHeight = getTextHeight(textCenter[getProgress()], textPaint);
